...
public:
  ...
  const Vector2F getPosition() const {
    return Vector2F(m_camera.getPosition());
  }
  float getRotation() const { return m_camera.getRoll(); }
  const Vector2F getUpDirection() const {
    return Vector2F(m_camera.getUpDirection());
  }
  const Vector2F getRightDirection() const {
    return Vector2F(m_camera.getRightDirection());
  }
  ...
  void setPosition(const Vector2F &position) {
    m_camera.setPosition(Vector3F(position, m_zPos));
  }

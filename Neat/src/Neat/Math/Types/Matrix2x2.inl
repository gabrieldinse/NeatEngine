...
inline constexpr Matrix<2, 2, T>::Matrix(const Matrix<2, 2, U> &m)
    : m_rows{RowType(m[0]), RowType(m[1])} {}
...
inline constexpr Matrix<2, 2, T>::Matrix(const Vector<2, V1> &row1,
    ...
    : m_rows{RowType(row1), RowType(row2)} {}
...
inline constexpr Matrix<2, 2, T> operator+(const Matrix<2, 2, T> &ma,
  ...
  return Matrix<2, 2, T>(ma[0] + mb[0], ma[1] + mb[1]);
}
...
template <typename T>
inline constexpr const typename Matrix<2, 2, T>::RowType &
Matrix<2, 2, T>::operator[](UInt32 row) const {
  return m_rows[row];
}

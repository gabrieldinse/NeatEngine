...
inline constexpr Matrix<4, 4, T>::Matrix(const Matrix<4, 4, U> &m)
    : m_rows{RowType(m[0]), RowType(m[1]), RowType(m[2]), RowType(m[3])} {}

template <typename T>
template <typename U>
inline constexpr Matrix<4, 4, T>::Matrix(const Matrix<3, 3, U> &m)
    : m_rows{RowType(m[0], 0), RowType(m[1], 0), RowType(m[2], 0),
             RowType(0, 0, 0, 1)} {}

template <typename T>
template <typename U>
inline constexpr Matrix<4, 4, T>::Matrix(const Matrix<2, 2, U> &m)
    : m_rows{RowType(m[0], 0, 0), RowType(m[1], 0, 0), RowType(0, 0, 1, 0),
             RowType(0, 0, 0, 1)} {}
...
inline constexpr Matrix<4, 4, T>::Matrix(const Vector<4, V1> &row1,
    ...
    : m_rows{RowType(row1), RowType(row2), RowType(row3), RowType(row4)} {}
...
inline constexpr Matrix<4, 4, T> operator+(const Matrix<4, 4, T> &ma,
  ...
  return Matrix<4, 4, T>(ma[0] + mb[0], ma[1] + mb[1], ma[2] + mb[2],
                         ma[3] + mb[3]);
}
...
inline constexpr Matrix<4, 4, T> operator*(const T &scalar,
  ...
  return Matrix<4, 4, T>(scalar * m[0], scalar * m[1], scalar * m[2],
                         scalar * m[3]);
}
...
inline constexpr Matrix<4, 4, T> operator/(const Matrix<4, 4, T> &m,
                                           const T &scalar) {
  return Matrix<4, 4, T>(m[0] / scalar, m[1] / scalar, m[2] / scalar,
                         m[3] / scalar);
}
...
template <typename T>
inline constexpr const typename Matrix<4, 4, T>::RowType &
Matrix<4, 4, T>::operator[](UInt32 row) const {
  return m_rows[row];
}

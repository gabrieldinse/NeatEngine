...
// Core log macros
   ...
   #define NT_ASSERT(x, ...) { if(!(x)) { NT_ERROR("Assertion Failed: ", __VA_ARGS__); __debugbreak(); } }
   #define NT_CORE_ASSERT(x, ...) { if(!(x)) { NT_CORE_ERROR("Assertion Failed: ", __VA_ARGS__); __debugbreak(); } }

...
struct TransformComponent {
  ...
  TransformComponent(const Vector2F &position,
                     ...
                     float rotation = 0.0f)
      : position(Vector3F{position, 0.0f}), scale(Vector3F{scale, 1.0f}), rotation(Vector3F{0.0f, 0.0f, rotation}) {}
  TransformComponent(const Vector3F &position,
                     const Vector2F &scale = {1.0f, 1.0f},
                     float rotation = 0.0f)
      : position(position), scale(Vector3F{scale, 1.0f}), rotation(Vector3F{0.0f, 0.0f, rotation}) {}

  Matrix4F getTransform() const {
    Matrix4F rotation =
        QuaternionF::fromEulerAngles(degreesToRadians(this->rotation)).toMatrix4();
